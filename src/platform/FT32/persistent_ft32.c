...
/*
 * Persistent data storage implementation using FT32F4 RTC backup registers.
...
{
    return RTC_GetBackUpReg(id);
}

void persistentObjectWrite(persistentObjectId_e id, uint32_t value)
{
    RTC_WriteProtectionCmd(DISABLE);
    RTC_BackUpRegConfig(id, value);
...
void persistentObjectInit(void)
{
    ...
    uint32_t wasSoftReset = RCC_GetFlagStatus(RCC_FLAG_REG_CSR, RCC_FLAG_SFTRSTF);
