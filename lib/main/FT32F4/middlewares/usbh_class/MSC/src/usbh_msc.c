...
/**@defgroup USBH_MSC_CORE_Private_TypeDefinitions
 * @{
 */
...
static USBH_StatusTypeDef  USBH_MSC_InterfaceInit(USBH_HandleTypeDef *phost)
{
  ...
  if ((MSC_Handle->InEp != 0U) && (MSC_Handle->InEpSize != 0U))
  {
    (void)USBH_OpenPipe(phost, MSC_Handle->InPipe, MSC_Handle->InEp,
                        phost->device.address, phost->device.speed,
                        USB_EP_TYPE_BULK, MSC_Handle->InEpSize);
  }
  else
  {
    return USBH_NOT_SUPPORTED;
 ...
 *         DeInitialize the Pipes used for the MSC class.
 * @param  phost: Host Handle
 * @retval USBH Status
 */
static USBH_StatusTypeDef  USBH_MSC_InterfaceDeInit(USBH_HandleTypeDef *phost)
{
  ...
  if (MSC_Handle->OutPipe != 0U)
  {
    ...
    MSC_Handle->OutPipe = 0U;    /* Reset the pipe as Free */
  }
  ...
  if (MSC_Handle->InPipe != 0U)
  {
    ...
    MSC_Handle->InPipe = 0U;    /* Reset the pipe as Free */
  }
 ...
 *         The function is responsible for handling Standard requests for MSC class.
 * @param  phost: Host Handle
 * @retval USBH Status
 */
static USBH_StatusTypeDef USBH_MSC_ClassRequest(USBH_HandleTypeDef *phost)
{
  ...
  switch (MSC_Handle->req_state)
  {
    ...
    case MSC_REQ_GET_MAX_LUN:
      ...
      /* when device do not support the getmaxlun request,
       * this should be considered as only one logical
       * unit is supported */
          ...
          case MSC_READ_CAPACITY10:
            ...
            if (scsi_status == USBH_OK)
            {
              if (MSC_Handle->unit[MSC_Handle->current_lun].state_changed == 1U)
              {
                ...
                USBH_UsrLog("Block number : %u Bytes",
                            (unsigned int)(MSC_Handle->unit[MSC_Handle->current_lun].capacity.block_nbr));
                USBH_UsrLog("Block Size   : %u Bytes",
                            (unsigned int)(MSC_Handle->unit[MSC_Handle->current_lun].capacity.block_size));
              }
...
/**
 * @brief  USBH_MSC_Read
 *         the function preforms a read operation
 * @param  phost: Host handle
 * @param  lun: logical unit number
 ...
 *         the function preforms a write operation
 * @param  phost: Host handle
 * @param  lun: logical unit number
 * @param  address: sector address
 * @param  pbuf: pointer to data
