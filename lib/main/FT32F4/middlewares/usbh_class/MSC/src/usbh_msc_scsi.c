...
/**@defgroup USBH_MSC_SCSI_Private_TypeDefinitions
 * @{
 */
 ...
 * @retval USBH Status
 */
USBH_StatusTypeDef  USBH_MSC_SCSI_ReadCapacity(USBH_HandleTypeDef *phost, uint8_t lun,
                                               SCSI_CapacityTypeDef *capacity)
{
  ...
  switch (MSC_Handle->hbot.cmd_state)
  {
    ...
    case BOT_CMD_WAIT:
      ...
      {
        /* assign the capacity */
        capacity->block_nbr = MSC_Handle->hbot.pbuf[3] |
                              ((uint32_t)MSC_Handle->hbot.pbuf[2] << 8U) |
                              ((uint32_t)MSC_Handle->hbot.pbuf[1] << 16U) |
                              ((uint32_t)MSC_Handle->hbot.pbuf[0] << 24U) ;
  ...
  switch (MSC_Handle->hbot.cmd_state)
  {
    ...
    case BOT_CMD_WAIT:
      ...
      if (error == USBH_OK)
      {
        ...
        (void)USBH_memcpy(inquiry->revision_id, &MSC_Handle->hbot.pbuf[32], 32U);
      }
      break;
 ...
 * @retval USBH Status
 */
USBH_StatusTypeDef  USBH_MSC_SCSI_RequestSense(USBH_HandleTypeDef *phost, uint8_t lun,
 ...
 * @retval USBH Status
 */
USBH_StatusTypeDef  USBH_MSC_SCSI_Write(USBH_HandleTypeDef *phost, uint8_t lun,
 ...
 * @retval USBH Status
 */
USBH_StatusTypeDef  USBH_MSC_SCSI_Read(USBH_HandleTypeDef *phost, uint8_t lun,
                                       uint32_t address, uint8_t *pbuf, uint32_t length)
{
  USBH_StatusTypeDef error = USBH_FAIL;
  MSC_HandleTypeDef *MSC_Handle = (MSC_HandleTypeDef *) phost->pActiveClass->pData;
