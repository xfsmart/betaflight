 ...
 * @brief This file includes the mass storage functions.
 * @{
 */

/**@defgroup USBH_MTP_PTP_Private_TypeDefinitions
 * @{
 */
...
/**@defgroup USBH_MTP_PTPE_Private_Variables
 * @{
 */
/**
 * @}
...
static void PTP_GetDevicePropValue(USBH_HandleTypeDef *phost, uint32_t *offset, uint32_t total,
 ...
 *         handle the PTP ptotocol.
 * @param  phost: Host Handle
 * @parma  lun : Logical unit number
 * @retval USBH Status
 */
USBH_StatusTypeDef USBH_PTP_Process(USBH_HandleTypeDef *phost)
{
  ...
  switch (MTP_Handle->ptp.state)
  {
    ...
    case PTP_OP_REQUEST_STATE:
      (void)USBH_BulkSendData(phost, ((uint8_t *)(void *) & (MTP_Handle->ptp.op_container)),
                              (uint16_t)MTP_Handle->ptp.op_container.length,
                              MTP_Handle->DataOutPipe, 1U);
    ...
    case PTP_DATA_OUT_PHASE_STATE:
      (void)USBH_BulkSendData(phost, MTP_Handle->ptp.data_ptr,
    ...
    case PTP_DATA_IN_PHASE_STATE:
      /* send first packet */
      (void)USBH_BulkReceiveData(phost, MTP_Handle->ptp.data_ptr,
    ...
    case PTP_RESPONSE_STATE:
      (void)USBH_BulkReceiveData(phost,
                                 ...
                                 PTP_USB_BULK_REQ_RESP_MAX_LEN, MTP_Handle->DataInPipe);
...
USBH_StatusTypeDef USBH_PTP_GetResponse(USBH_HandleTypeDef *phost, PTP_ContainerTypedef *resp)
{
  ...
  /* build appropriate ptp container */
...
/**
 * @brief  PTP_BufferFullCallback
 *         informs user that data buffer is full
 * @param  phost: Host Handle
 * @retval none
 */
static void PTP_BufferFullCallback(USBH_HandleTypeDef *phost)
 ...
 *         get storage ids and fills stor_ids structure.
 * @param  phost: Host Handle
 * @param  stor_ids: storage ids structure
 * @retval none
 */
static void PTP_GetStorageIDs(USBH_HandleTypeDef *phost, PTP_StorageIDsTypedef *stor_ids)
{
 ...
 *         get storage info and fills stor_info structure.
 * @param  phost: Host Handle
 * @param  stor_info: storage info structure
 * @retval none
 */
static void PTP_GetStorageInfo(USBH_HandleTypeDef *phost, uint32_t storage_id, PTP_StorageInfoTypedef *stor_info)
{
 ...
 *         get object info and fills object_info structure.
 * @param  phost: Host Handle
...
static void PTP_GetDevicePropValue(USBH_HandleTypeDef *phost, uint32_t *offset,
                                   uint32_t total, PTP_PropertyValueTypedef *value,
                                   uint16_t datatype)
{
  MTP_HandleTypeDef *MTP_Handle = (MTP_HandleTypeDef *)phost->pActiveClass->pData;
  uint8_t *data = MTP_Handle->ptp.data_container.payload.data;
  ...
  switch (datatype)
  {
    ...
    case PTP_DTC_UINT8:
      value->u8 = *(uint8_t *)(void *) & (data[*offset]);
      *offset += 1U;
      break;
...
/**
 * @brief  PTP_GetString
 *         get SCII string from
 * @param  str: assii string
 ...
 *         get device prop descriptor
 * @param  phost: host handle
 * @retval USBH Status
 */
USBH_StatusTypeDef USBH_PTP_GetDevicePropDesc(USBH_HandleTypeDef *phost, uint16_t propcode,
 ...
 *         get device info dataset and fills deviceinfo structure
 * @param  phost: host handle
 * @param  dev_info: device info structure
 * @retval USBH Status
 */
...
/**
 * @brief  USBH_PTP_GetStorageInfo
 *         get storage info
 * @param  phost: host handle
 * @retval USBH Status
 */
USBH_StatusTypeDef USBH_PTP_GetStorageInfo(USBH_HandleTypeDef *phost, uint32_t storage_id,
 ...
 * @retval USBH Status
 */
USBH_StatusTypeDef USBH_PTP_GetNumObjects(USBH_HandleTypeDef *phost, uint32_t storage_id,
 ...
 * @retval USBH Status
 */
USBH_StatusTypeDef USBH_PTP_GetObjectHandles(USBH_HandleTypeDef *phost, uint32_t storage_id,
                                             uint32_t objectformatcode, uint32_t associationOH,
 ...
 * @retval USBH Status
 */
USBH_StatusTypeDef USBH_PTP_GetObjectInfo(USBH_HandleTypeDef *phost, uint32_t handle,
 ...
 * @retval USBH Status
 */
USBH_StatusTypeDef USBH_PTP_DeleteObject(USBH_HandleTypeDef *phost, uint32_t handle,
 ...
 * @retval USBH Status
 */
USBH_StatusTypeDef USBH_PTP_GetObject(USBH_HandleTypeDef *phost, uint32_t handle,
                                      uint8_t *object)
{
  USBH_StatusTypeDef status = USBH_BUSY;
  MTP_HandleTypeDef *MTP_Handle = (MTP_HandleTypeDef *)phost->pActiveClass->pData;
 ...
 * @retval USBH Status
 */
USBH_StatusTypeDef USBH_PTP_GetPartialObject(USBH_HandleTypeDef *phost, uint32_t handle,
 ...
 * @retval USBH Status
 */
USBH_StatusTypeDef USBH_PTP_GetObjectPropsSupported(USBH_HandleTypeDef *phost, uint16_t ofc,
 ...
 * @retval USBH Status
 */
USBH_StatusTypeDef USBH_PTP_GetObjectPropDesc(USBH_HandleTypeDef *phost, uint16_t opc,
                                              uint16_t ofc, PTP_ObjectPropDescTypeDef *opd)
{
  USBH_StatusTypeDef status = USBH_BUSY;
  MTP_HandleTypeDef *MTP_Handle = (MTP_HandleTypeDef *)phost->pActiveClass->pData;
  PTP_ContainerTypedef ptp_container;
 ...
 * @retval USBH Status
 */
USBH_StatusTypeDef USBH_PTP_GetObjectPropList(USBH_HandleTypeDef *phost, uint32_t handle,
                                              MTP_PropertiesTypedef *pprops, uint32_t *nrofprops)
{
  ...
  switch (MTP_Handle->ptp.req_state)
  {
    case PTP_REQ_SEND:
      ...
      ptp_container.Param5 = 0xFFFFFFFFU; /* Return fill tree below the param1 handle */
 ...
 * @retval USBH Status
 */
USBH_StatusTypeDef USBH_PTP_SendObject(USBH_HandleTypeDef *phost, uint32_t handle,
