/**
  ...
  * @file    			usbh_core.c
  * @author  			FMD XA
  * @brief   			This file implements the functions for the core state machine
  *               process the enumeration and the control transfer process
  * @version 			V1.0.0           
  * @data		 			2025-04-10
  ******************************************************************************
  */
...
/**@defgroup USBH_CORE
 * @brief This file handles the basic enumeration when a device is connected to the host.
 * @{
 */
 ...
 * @retval USBH Status
 */
USBH_StatusTypeDef  USBH_Init(USBH_HandleTypeDef *phost, uint8_t core_id, 
                              uint8_t id, USBH_ClassTypeDef *pclass,
...
{
  /* stop and clearup the low level driver */
  (void)USBH_LL_Stop(phost);
  ...
  switch (phost->gState)
  {
    case HOST_IDLE :
      if ((phost->device.is_connected) != 0U)
      {
        ...
        /* wait for 200ms after connection */
    ...
    case HOST_DEV_ATTACHED :
      ...
      phost->device.speed = (uint8_t)USBH_LL_GetSpeed();
      ...
      /* open control pipes in */
      (void)USBH_OpenPipe(phost, phost->Control.pipe_in, 0x80U,
                          phost->device.address, phost->device.speed,
                          USBH_EP_CONTROL, (uint16_t)phost->Control.pipe_size);

      /* open control pipes out */
    ...
    case HOST_CLASS_REQUEST :
      ...
      if (phost->pActiveClass != NULL)
      {
        ...
        {
          phost->gState = HOST_ABORT_STATE;
          USBH_UsrLog("Device not responding Please unplug.");
        }
        else
        {
          /*...*/
...
static USBH_StatusTypeDef USBH_HandleEnum(USBH_HandleTypeDef *phost)
{
  ...
  {
    case ENUM_IDLE :
      /* get device desc for only 1st 8bytes : to get ep0 maxpacketsize */
      ...
      if (ReqStatus == USBH_OK)
      {
        ...
        /* modify control channel configuration for MaxPacket size */
        (void)USBH_OpenPipe(phost, phost->Control.pipe_in, 0x80U, phost->device.address,
                            phost->device.speed, USBH_EP_CONTROL,
                            (uint16_t)phost->Control.pipe_size);
        /* open control pipe out */
    ...
    case ENUM_GET_FULL_DEV_DESC :
      ...
      else if (ReqStatus == USBH_NOT_SUPPORTED)
      {
        ...
        {
          /* buggy Device can't complete get device desc request */
          USBH_UsrLog("Control error, Device not responding Plesae unplug the device");
          phost->gState = HOST_ABORT_STATE;
        }
        else
        {
