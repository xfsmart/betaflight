...
static uint32_t MTP_build_data_proplist(USBD_HandleTypeDef *pdev, MTP_PropertiesListTypedef proplist,
                                        uint32_t idx);
...
/**
 * @brief  USBD_MTP_OPT_CreaeObjectHandle
 *         Open a new session.
 * @param  pdev: device instance
 * @retval None
 */
void USBD_MTP_OPT_CreaeObjectHandle(USBD_HandleTypeDef *pdev)
{
 ...
 *         Send object information from host to MTP device
 * @param  pdev: device instance
 * @retval None
 */
void USBD_MTP_OPT_SendObjectInfo(USBD_HandleTypeDef *pdev, uint8_t *buff, uint32_t len)
{
  ...
  switch (hmtp->RECEIVE_DATA_STATUS)
  {
    ...
    case RECEIVE_FIRST_DATA:
      tmp = buff;
      (void)USBD_memcpy(ObjInfo_buff, tmp + 12U, (uint16_t)(hmtp->MaxPcktLen - MTP_CONT_HEADER_SIZE));
      hmtp->RECEIVE_DATA_STATUS = RECEIVE_REST_OF_DATA;
      break;
...
static void MTP_Get_DeviceInfo(void)
{
  ...
  MTP_DeviceInfo.EventsSupported_len = SUPP_EVENTS_LEN;  /* event that are currently genreated by the device */
...
static void MTP_Get_StorageInfo(USBD_HandleTypeDef *pdev)
{
  ...
  MTP_StorageInfo.FreeSpaceInObjects = FREE_SPACE_IN_OBJ_NOT_USED;
...
static void MTP_Get_ObjectPropList(USBD_HandleTypeDef *pdev)
{
  ...
  (void)USBD_memcpy(hmtp->GenericContainer.data, (const uint8_t *)&MTP_PropertiesList.MTP_Properties_len,
                    hmtp->ResponseLength);
...
static void MTP_Get_ObjectInfo(USBD_HandleTypeDef *pdev)
{
  ...
  MTP_ObjectInfo.ThumbPixWidth        = 0U;
...
static uint32_t MTP_build_data_propdesc(USBD_HandleTypeDef *pdev, MTP_ObjectPropDescTypeDef def)
{
  ...
  (void)USBD_memcpy(hmtp->GenericContainer.data + dataLength,
                    (const uint8_t *)&MTP_ObjectPropDesc.GroupCode,
                    sizeof(MTP_ObjectPropDesc.GroupCode));
  dataLength += sizeof(MTP_ObjectPropDesc.GroupCode);

  (void)USBD_memcpy(hmtp->GenericContainer.data + dataLength,
                    (const uint8_t *)&MTP_ObjectPropDesc.FormFlag,
 ...
 * @retval None
 */
static uint32_t MTP_build_data_proplist(USBD_HandleTypeDef *pdev, MTP_PropertiesListTypedef proplist, uint32_t idx)
{
  USBD_MTP_HandleTypeDef *hmtp = (USBD_MTP_HandleTypeDef *)pdev->pClassDataCmsit[pdev->classId];
...
static uint32_t MTP_build_data_ObjInfo(USBD_HandleTypeDef *pdev, MTP_ObjectInfoTypeDef objinfo)
{
  ...
  (void)USBD_memcpy(hmtp->GenericContainer.data,
                    (const uint8_t *)&objinfo, ObjInfo_len);
