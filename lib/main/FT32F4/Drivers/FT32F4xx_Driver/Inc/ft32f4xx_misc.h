...
/* HAL_StatusTypeDef definition for HAL compatibility */
typedef enum {
    HAL_OK       = 0x00U,
    HAL_ERROR    = 0x01U,
    HAL_BUSY     = 0x02U,
...
HAL_StatusTypeDef __attribute__((weak)) InitTick(uint32_t TickPriority);
void __attribute__((weak)) IncTick(void);
__attribute__((weak)) void HAL_Delay(uint32_t Delay);
uint32_t __attribute__((weak))  GetTick(void);
