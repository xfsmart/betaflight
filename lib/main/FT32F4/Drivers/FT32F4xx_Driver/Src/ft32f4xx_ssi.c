...
HAL_StatusTypeDef SSI_Fifo0Tx(SSI_HandleTypeDef *ssi, SSI_TxInitTypeDef *ssi_txinit,uint32_t Timeout,uint32_t tickstart);
...
/**
  ...
  *
  */
HAL_StatusTypeDef SSI_Transmit(SSI_HandleTypeDef *ssi, \
                  SSI_InitTypeDef *ssi_init, \
                  SSI_TxInitTypeDef *ssi_txinit, \
                  uint8_t *pData0,\
                  uint8_t *pData1,\
...
HAL_StatusTypeDef SSI_Fifo0Tx(SSI_HandleTypeDef *ssi, SSI_TxInitTypeDef *ssi_txinit,uint32_t Timeout,uint32_t tickstart)
{
   ...
   else if((Timeout != MAX_DELAY) && ((Timeout == 0U) || ((GetTick() - tickstart) > Timeout)))
   {
      /* Update error code */
