...
void RCC_GetClocksFreq(RCC_ClocksTypeDef* RCC_Clocks)
{
  ...
  /* Suppress unused variable warnings for FT32F4 */
  (void)pll2mul; (void)pll2nul; (void)pll2vcoclk; (void)hpresc; (void)presc1; (void)presc2;
  ...
  if (pllsource == 0x00)
  {
   /* HSI oscillator clock divided by pllmul and multiplicat by pllnul as pllvcoclk output */
   pllvcoclk  = (HSI_VALUE * pllnul)/pllmul;
   if(pllq==0){ RCC_Clocks->PLLQ_Frequency = pllvcoclk/2;       }
   else       { RCC_Clocks->PLLQ_Frequency = pllvcoclk/(pllq+1);}
   if(pllr==0){ RCC_Clocks->PLLR_Frequency = pllvcoclk/2;       }
   else       { RCC_Clocks->PLLR_Frequency = pllvcoclk/(pllr+1);}
  }
  else
  {
   /* HSE oscillator clock divided by pllmul and multiplicat by pllnul as pllvcoclk output */
  ...
  if (pll2source == 0x00)
  {
   ...
   /* pll2vcoclk  = (HSI_VALUE * pll2nul)/pll2mul; */ // FT32F4: Not used
   if(pll2q==0){ RCC_Clocks->PLLQ_Frequency = pllvcoclk/2;        }
   else        { RCC_Clocks->PLLQ_Frequency = pllvcoclk/(pll2q+1);}
   if(pll2r==0){ RCC_Clocks->PLLR_Frequency = pllvcoclk/2;        }
   else        { RCC_Clocks->PLLR_Frequency = pllvcoclk/(pll2r+1);}
  }
  else
  {
   /* HSE oscillator clock divided by pll2mul and multiplicat by pll2nul as pll2vcoclk output */
   /* pll2vcoclk  = (HSE_VALUE * pll2nul)/pll2mul; */ // FT32F4: Not used
   if(pll2q==0){ RCC_Clocks->PLLQ_Frequency = pllvcoclk/2;        }
   else        { RCC_Clocks->PLLQ_Frequency = pllvcoclk/(pll2q+1);}
   if(pll2r==0){ RCC_Clocks->PLLR_Frequency = pllvcoclk/2;        }
   else        { RCC_Clocks->PLLR_Frequency = pllvcoclk/(pll2r+1);}
