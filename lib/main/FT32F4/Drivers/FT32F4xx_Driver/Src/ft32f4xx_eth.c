...
uint32_t ETH_Init(ETH_HandleTypeDef *heth)
{
  ...
  /*while ((heth->Instance->DMAMR, ETH_DMAMR_SWR) > 0U)
    {
...
uint32_t ETH_ReadData(ETH_HandleTypeDef *heth, void **pAppBuff)
{
  ...
  while((((dmarxdesc->DESC3)&ETH_DMARXNDESCWBF_OWN) == (uint32_t)(0x0U)) && (desccnt < desccntmax)
        && (rxdataready == 0U))
  {
                                          ...
                                          uint32_t *pRegValue)
{
  ...
  MODIFY_REG(tmpreg,ETH_MAC_MDIOADDR_RDA,(PHYAddr << 16));
...
static void ETH_MACDMAConfig(ETH_HandleTypeDef *heth)
{
  ...
  macDefaultConf.Speed = ETH_SPEED_10M;
...
static void ETH_DMARxDescListInit(ETH_HandleTypeDef *heth)
{
  ...
  WRITE_REG(heth->Instance->ETH_DMA_TXDESCTAILPTR, ((uint32_t)(heth->Init.RxDesc + (uint32_t)(ETH_RX_DESC_CNT - 1U))));
}
